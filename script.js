...
function updateTopSummary(){
  ...
  const dateEl = document.getElementById("dateSummaryLine");
  if(statusFilter === "open"){
    if(dateEl.firstChild) dateEl.firstChild.textContent = "on " + ordinal(selectedDate) + " September";
    document.getElementById("countSummaryLine").textContent = n + " " + typeLabel + "Studio" + (n===1?"":"s") + " open";
  } else {
    if(dateEl.firstChild) dateEl.firstChild.textContent = "in the trail on " + ordinal(selectedDate) + " September";
