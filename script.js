...
function buildRoutePlannerSheet(){
  ...
  if(currentRouteSelection.isEditing){
    ...
    body.innerHTML = `
      <div style="padding:16px;">
        <div style="margin-bottom:16px;">
          ...
          <input type="text" id="routeStartPostcode" placeholder="Enter postcode" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px; box-sizing:border-box; font-size:14px; margin-bottom:6px;">
          <button id="routeUseCurrentBtn" style="width:100%; padding:10px; background:#2e8b3d; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:500;">📍 Use Current Location</button>
        ...
        <div style="margin-bottom:16px;">
          ...
          <input type="text" id="routeEndPostcode" placeholder="Enter postcode" style="width:100%; padding:10px; border:1px solid #ccc; border-radius:4px; box-sizing:border-box; font-size:14px;">
        </div>
        ...
        <button id="routeOptimizeBtn" style="width:100%; padding:12px; background:#ff5a5f; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:bold;">Optimize & Save Route</button>
      </div>
    `;
    ...
    document.getElementById("routeStartPostcode").onchange = function(){
      const postcode = this.value.trim();
      if(postcode){
        currentRouteSelection.startPoint = { postcode: postcode };
        document.getElementById("routeStartStatus").textContent = "✓ Postcode set: " + postcode;
      }
    };
...
function setupVenueSearch(container, allVenues){
  ...
  searchInput.oninput = function(){
    ...
    // Filter venues by venue number or artist name
    const matches = allVenues.filter(function(v){
      const venueNum = String(v.venue).toLowerCase();
      // Get all artist names for this venue
      const artistNames = (v.artists || []).map(function(a){ return a.name.toLowerCase(); }).join(" ");
      return venueNum.includes(query) || artistNames.includes(query);
    });
    ...
    if(matches.length > 0){
      matches.forEach(function(v){
        ...
        const artistList = (v.artists || []).map(function(a){ return a.name; }).join(", ");
        resultDiv.innerHTML = `<strong>Venue ${v.venue}</strong><br><small style="color:#666;">${artistList}</small>`;
...
function buildSelectedVenuesList(container){
  ...
  if(currentRouteSelection.venues.length === 0){
    container.innerHTML = "<small style='color:#999;'>No venues selected yet</small>";
    return;
  }
  ...
  currentRouteSelection.venues.forEach(function(v, i){
    ...
    const artistList = (v.artists || []).map(function(a){ return a.name; }).join(", ");
    label.textContent = (i + 1) + ". Venue " + v.venue + " (" + artistList + ")";
