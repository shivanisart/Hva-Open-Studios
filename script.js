...
// visitor) so they can be shown on the buttons, e.g. "â¤ï¸ Love (12)" /
// "â Visited (34)". Requires the *_counts views + grants from
...
function initGeolocation(){
  ...
  if(!navigator.geolocation){
    statusEl.textContent = "ð Location not supported by this browser";
  ...
  navigator.geolocation.watchPosition(
    ...
    function(err){
      userLocation = null;
      statusEl.textContent = err.code === 1 ? "ð Location permission denied" : "ð Location unavailable";
      statusEl.style.display = "block";
      updateUserDot();
    },
...
function updateTopSummary(){
  ...
  document.getElementById("artTypeFilterBtn").textContent = typeFilter.size === 0 ? "Art type â¾" : (typeFilter.size === 1 ? Array.from(typeFilter)[0] : "Multiple â¾");
    ...
    if(supabaseClient && currentUserId){
      supabaseClient.from("profiles").upsert(
        ...
        document.getElementById("trailEmailRow").innerHTML = res.error
          ? "<div class=\"trailNote\">Sorry, couldn't save that â please try again.</div>"
  ...
  html += "" +
    "<div class=\"routingSection\">" +
      ...
      "<div class=\"routingRow\">" +
        "<a class=\"directionsBigBtn\" href=\"" + directionsUrl + "\" target=\"_blank\" rel=\"noopener\">ðºï¸ Route on Google Maps</a>" +
      "</div>" +
...
function renderCommentList(listEl, notes){
  if(!notes || notes.length === 0){
    listEl.innerHTML = "<div class=\"inlineCommentItem\" style=\"font-style:italic;color:#999;\">No comments yet â be the first!</div>";
  } else {
...
async function renderInlineComments(listEl, artistName){
  ...
  listEl.innerHTML = "<div class=\"inlineCommentItem\" style=\"font-style:italic;color:#999;\">Loading commentsâ¦</div>";
      ...
      Object.entries(today).forEach(function(entry){
        ...
        row.innerHTML =
          ...
          "<div class=\"listRowActions\">" +
            "<a class=\"directionsBigBtn\" href=\"" + getDirectionsUrl(loc.lat, loc.lng) + "\" target=\"_blank\" rel=\"noopener\">ðºï¸ Directions</a>" +
