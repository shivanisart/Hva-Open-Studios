...
if(document.getElementById("planVisitsBtn")){
  document.getElementById("planVisitsBtn").onclick = openRoutePlanner;
}

// Fix close button - attach multiple ways to ensure it works
function setupRoutePlannerClose(){
  const closeBtn = document.getElementById("routePlannerClose");
  if(closeBtn){
    closeBtn.onclick = closeAllSheets;
    closeBtn.addEventListener("click", closeAllSheets, true); // capture phase
  }
  
  // Also make scrim close the route planner
...
// Call this after DOM is ready AND after each sheet opens
setTimeout(setupRoutePlannerClose, 500);

// Also attach after opening route planner
const origOpenRoutePlanner = openRoutePlanner;
openRoutePlanner = function(){
  origOpenRoutePlanner();
  setTimeout(setupRoutePlannerClose, 100);
};
...
function setupVenueSearch(container, allVenues){
  ...
  searchInput.oninput = function(){
    ...
    const matches = allVenues.filter(function(v){
      ...
      // Get all artist names - handle different data structures
      let artistNames = "";
      if(v.artists && Array.isArray(v.artists)){
        artistNames = v.artists.map(function(a){ 
          return (a.name || a || "").toLowerCase(); 
        }).join(" ");
      } else if(v.artist_names){
        artistNames = (v.artist_names || "").toLowerCase();
      }
    ...
    if(matches.length > 0){
      matches.forEach(function(v){
        ...
        // Get artist names - handle different structures
        let artistList = "";
        if(v.artists && Array.isArray(v.artists)){
          artistList = v.artists.map(function(a){ return a.name || a || ""; }).join(", ");
        } else if(v.artist_names){
          artistList = v.artist_names;
        }
...
async function saveRoute(){
  ...
  if(!currentUserId){
    alert("ERROR: You must be logged in to save routes. Please log in at the top of the app.");
    return;
  }
