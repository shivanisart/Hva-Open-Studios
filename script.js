...
function openRoutePlanner(){
  console.log("openRoutePlanner called");
  closeAllSheets();
  
  // First, just open the empty sheet
  const sheet = document.getElementById("routePlannerSheet");
  if(!sheet){
    alert("ERROR: routePlannerSheet not found in HTML");
    return;
  }
  ...
  // Then build the content after a tiny delay
  setTimeout(function(){
    try {
      buildRoutePlannerSheet();
    } catch(err) {
      alert("Error building route planner: " + err.message);
    }
  }, 100);
}
