...
function avatarHtmlWithVenue(name, venue, extraClass){
  const cls = "artistAvatar" + (extraClass ? (" " + extraClass) : "");
  const info = ARTIST_INFO[name];
  ...
  results.forEach(function(r){
    ...
    row.innerHTML =
      "<div style=\"display:flex;align-items:center;gap:8px;\">" +
        avatarHtmlWithVenue(r.artistName, r.loc.venue, "srAvatar") +
        "<div>" +
...
function openPanel(loc, focusArtist){
  ...
  artistsToShow.forEach(function(artist){
    ...
    const profileUrl = loc.profileUrl || info.profileUrl;
    ...
    const photoUrl = (info.venuePhotos && info.venuePhotos[loc.venue]) ? info.venuePhotos[loc.venue] : info.photo;
    const bigPhotoHtml = photoUrl
      ? ("<img src=\"" + photoUrl + "\" class=\"panelArtistPhoto\" alt=\"\" loading=\"lazy\" " +
